...
def create_post_project_tab():
    ...
    with st.form("post_project_form"):
        title = st.text_input("Project Title:*", key="dev_title")
        location = st.text_input("Location:*", key="dev_location")
        project_type = st.selectbox("Project Type:*", [
                ...
                "Infrastructure",
                "Renovation"
            ], key="dev_project_type")
        budget = st.text_input("Budget (USD):*", key="dev_budget")
        description = st.text_area("Description:*", key="dev_description")
        uploaded_files = st.file_uploader("Upload Project Files", accept_multiple_files=True, key="dev_files")
                ...
                if project['files']:
                    ...
                    for file_info in project['files']:
                        st.download_button(label=file_info['name'], data=file_info['data'],
                                           file_name=file_info['name'], key=f"download_{file_info['name']}_{project['title']}")
                else:
...
def create_available_projects_tab():
    ...
    search_text = st.text_input("Search", key="contractor_projects_search")
    selected_location = st.selectbox("Location", ['All', 'Panama City', 'Costa del Este', 'Obarrio',
                        'San Francisco', 'Punta Pacifica', 'Clayton', 'Other'], key="contractor_projects_location")
    selected_type = st.selectbox("Project Type", [
        ...
        "Infrastructure",
        "Renovation"
    ], key="contractor_projects_type")
    min_budget = st.text_input("Min Budget (USD)", key="contractor_projects_min_budget")
    max_budget = st.text_input("Max Budget (USD)", key="contractor_projects_max_budget")
    projects = st.session_state.projects
    filtered_projects = []
    try:
        ...
        for project in filtered_projects:
            with st.expander(project['title']):
                ...
                with st.form(f"bid_form_{project['title']}"):
                    amount = st.text_input("Bid Amount ($):*", key=f"bid_amount_{project['title']}")
                    timeline = st.text_input("Timeline (days):*", key=f"bid_timeline_{project['title']}")
                    company = st.text_input("Company Name:*", key=f"bid_company_{project['title']}")
                    contact = st.text_input("Contact Person:*", key=f"bid_contact_{project['title']}")
                    phone = st.text_input("Phone:*", key=f"bid_phone_{project['title']}")
                    email = st.text_input("Email:*", key=f"bid_email_{project['title']}")
                    license = st.text_input("License Number:*", key=f"bid_license_{project['title']}")
                    approach = st.text_area("Project Approach:*", key=f"bid_approach_{project['title']}")
                    experience = st.text_area("Similar Projects Experience:*", key=f"bid_experience_{project['title']}")
                    bid_files = st.file_uploader("Upload Supporting Documents", accept_multiple_files=True, key=f"bid_files_{project['title']}")
...
def create_my_bids_tab():
    ...
    status_filter = st.selectbox("Status", ['All', 'Submitted', 'Under Review', 'Awarded', 'Not Selected', 'Pending'], key="my_bids_status")
    bids = []
            ...
            with st.expander(f"Bid for {bid['project_title']}"):
                ...
                if bid['files']:
                    ...
                    for file_info in bid['files']:
                        st.download_button(label=file_info['name'], data=file_info['data'],
                                           file_name=file_info['name'], key=f"download_bid_file_{file_info['name']}_{bid['project_title']}")
    else:
        st.write("No bids found")

def create_materials_search_tab():
    st.subheader("Materials Search")
    search_text = st.text_input("Search", key="materials_search_text")
    category = st.selectbox("Category", ['All Categories'] + list(st.session_state.categories.keys()), key="materials_search_category")
    subcategory = 'All'
    if category != 'All Categories':
        subcategories = st.session_state.categories.get(category, [])
        subcategory = st.selectbox("Subcategory", ['All'] + subcategories, key="materials_search_subcategory")
    location = st.selectbox("Location", ['All', 'Panama City', 'Costa del Este', 'Obarrio',
                                      'San Francisco', 'Punta Pacifica', 'Clayton'], key="materials_search_location")
    availability = st.selectbox("Availability", ['All', 'In Stock', 'Limited Stock', 'Out of Stock'], key="materials_search_availability")
    min_price = st.text_input("Min Price (USD)", key="materials_search_min_price")
    max_price = st.text_input("Max Price (USD)", key="materials_search_max_price")
        ...
        for material in filtered_materials:
            with st.expander(material['name']):
                ...
                with st.form(f"order_form_{material['name']}"):
                    quantity = st.text_input("Quantity:*", value=str(material['minimum_order']), key=f"order_quantity_{material['name']}")
                    delivery_date = st.date_input("Delivery Date:*", min_value=datetime.datetime.now().date(), key=f"order_delivery_date_{material['name']}")
                    delivery_address = st.text_area("Delivery Address:*", key=f"order_delivery_address_{material['name']}")
                    project_name = st.text_input("Project Name:*", key=f"order_project_name_{material['name']}")
                    instructions = st.text_area("Special Instructions:", key=f"order_instructions_{material['name']}")
                    contact_person = st.text_input("Contact Person:*", key=f"order_contact_person_{material['name']}")
                    contact_phone = st.text_input("Contact Phone:*", key=f"order_contact_phone_{material['name']}")
...
def create_orders_tab():
    ...
    status_filter = st.selectbox("Status", ['All', 'Pending', 'Confirmed', 'In Transit', 'Delivered', 'Cancelled'], key="my_orders_status")
...
def create_add_materials_tab():
    ...
    with st.form("add_material_form"):
        name = st.text_input("Material Name:*", key="add_material_name")
        category = st.selectbox("Category:*", list(st.session_state.categories.keys()), key="add_material_category")
        subcategories = st.session_state.categories.get(category, [])
        subcategory = st.selectbox("Subcategory:*", subcategories, key="add_material_subcategory")
        price = st.text_input("Price (USD):*", key="add_material_price")
        min_order = st.text_input("Minimum Order Quantity:*", key="add_material_min_order")
        location = st.text_input("Location:*", key="add_material_location")
        contact = st.text_input("Contact Number:*", key="add_material_contact")
        availability = st.selectbox("Availability:*", [
                "In Stock",
                "Limited Stock",
                "Out of Stock",
                "Available on Order"
            ], key="add_material_availability")
...
def create_my_materials_tab():
    ...
    category_filter = st.selectbox("Category", ['All'] + list(st.session_state.categories.keys()), key="my_materials_category")
    availability_filter = st.selectbox("Availability", ['All', 'In Stock', 'Limited Stock', 'Out of Stock'], key="my_materials_availability")
        ...
        for material in filtered_materials:
            with st.expander(material['name']):
                ...
                with st.form(f"edit_material_form_{material['name']}"):
                    price = st.text_input("Price (USD):*", value=str(material['price']), key=f"edit_material_price_{material['name']}")
                    min_order = st.text_input("Minimum Order Quantity:*", value=str(material['minimum_order']), key=f"edit_material_min_order_{material['name']}")
                    availability = st.selectbox("Availability:*", [
                            "In Stock",
                            "Limited Stock",
                            "Out of Stock",
                            "Available on Order"
                        ], index=["In Stock", "Limited Stock", "Out of Stock", "Available on Order"].index(material['availability']), key=f"edit_material_availability_{material['name']}")
...
def create_supplier_orders_tab():
    ...
    status_filter = st.selectbox("Status", ['All', 'Pending', 'Confirmed', 'In Transit', 'Delivered', 'Cancelled'], key="supplier_orders_status")
        ...
        for order in filtered_orders:
            with st.expander(f"Order {order['order_id']} - {order['material']}"):
                ...
                with st.form(f"update_status_form_{order['order_id']}"):
                    new_status = st.selectbox("Update Status", ['Pending', 'Confirmed', 'In Transit', 'Delivered', 'Cancelled'],
                                              index=['Pending', 'Confirmed', 'In Transit', 'Delivered', 'Cancelled'].index(order['status']), key=f"update_order_status_{order['order_id']}")
